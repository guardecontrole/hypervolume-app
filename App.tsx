            ...
            <section className={`bg-slate-900 border rounded-2xl overflow-hidden shadow-2xl transition-colors ${isDeloadActive ? 'border-emerald-500/30' : 'border-slate-800'}`}>
              ...
              <div className="overflow-x-auto scrollbar-thin">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-900 text-[10px] uppercase font-black text-slate-500 sticky top-0 z-20">
                    <tr>
                      ...
                      (Object.entries(groupedPlan) as [string, PlanItem[]][]).map(([category, items]) => {
                        ...
                        return (
                          <React.Fragment key={category}>
                            ...
                            >
                              <td className={`p-4 sticky left-0 bg-slate-950 group-hover:bg-slate-900 z-20 font-black text-xs flex items-center gap-3 shadow-[4px_0_8px_rgba(0,0,0,0.15)] transition-colors ${isDeloadActive ? 'text-emerald-300' : 'text-indigo-300'}`}>
                              ...
                              <td className={`p-4 text-center font-black text-xs sticky left-52 bg-slate-950 group-hover:bg-slate-900 z-20 shadow-[4px_0_8px_rgba(0,0,0,0.15)] transition-colors ${isDeloadActive ? 'text-emerald-400/60' : 'text-indigo-400/60'}`}>
                                {categorySeries}S
                              </td>
                            ...
                            {!isCollapsed && items.map(item => {
                              ...
                              return (
                                <React.Fragment key={item.id}>
                                  <tr className={`group transition-all cursor-pointer ${isExpanded ? 'bg-slate-800/60' : 'bg-slate-900'} hover:bg-slate-800/50 ${focusedPlanExerciseId && !isRowFocused ? 'opacity-30 grayscale' : ''}`} onClick={() => toggleExpandExercise(item.id)}>
                                    <td className={`p-4 w-52 font-bold text-sm sticky left-0 z-10 flex items-center gap-2 shadow-[4px_0_8px_rgba(0,0,0,0.15)] transition-colors ${isExpanded ? 'bg-slate-800' : 'bg-slate-900'} group-hover:bg-slate-800`}>
                                    ...
                                    <td className={`p-4 w-20 sticky left-52 z-10 shadow-[4px_0_8px_rgba(0,0,0,0.15)] transition-colors ${isExpanded ? 'bg-slate-800' : 'bg-slate-900'} group-hover:bg-slate-800`} onClick={(e) => e.stopPropagation()}>
                                      <input 
                                        type="number" 
                                        value={item.series || ''} 
                                    ...
                                    <td className={`p-4 sticky right-0 text-center transition-colors ${isExpanded ? 'bg-slate-800' : 'bg-slate-900'} group-hover:bg-slate-800`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="number" value={strengthInputs.bw || ''} onFocus={(e) => e.target.select()} onChange={(e) => setStrengthInputs(prev => ({ ...prev,
