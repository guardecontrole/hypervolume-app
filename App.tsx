...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'strength' | 'plan' | 'workouts' | 'analysis' | 'history' | 'periodization'>('plan');
  ...
  useEffect(() => {
    ...
    const savedCustom = localStorage.getItem('hv_custom_exercises');
  ...
  const saveCustomExercise = useCallback(() => {
      ...
      const mappedMuscles = Object.entries(newCustomExercise.muscles)
          .filter(([, contribution]) => contribution > 0)
          .map(([name, contribution]) => ({
              name,
              type: MUSCULOS_GRANDES.includes(name) ? 'principal' as const : 'sinergista' as const,
  ...
  return (
    <div className={`min-h-screen pb-24 md:pb-20 transition-colors duration-500 ${isDeloadActive ? 'bg-slate-950' : 'bg-slate-950'}`}>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
           <div className="bg-slate-900 border border-slate-700 w-full max-w-xl rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto no-scrollbar">
               <h3 className="text-2xl font-black mb-8 uppercase tracking-tighter flex justify-between items-center text-white">
               ...
               <button onClick={() => setShowSettings(false)} className="bg-indigo-600 text-white px-4 py-4 rounded-xl mt-8 w-full font-black uppercase text-xs tracking-widest shadow-lg hover:bg-indigo-500 transition-colors">Salvar e Fechar</button>
           </div>
        </div>
      )}
                          ...
                          <div key={muscle} className={`p-3 rounded-xl border flex flex-col justify-between transition-colors ${(newCustomExercise.muscles[muscle] || 0) > 0 ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-slate-950/50 border-slate-800'}`}>
                              ...
                              <div className="relative">
                                <input 
                                  ...
                                  value={newCustomExercise.muscles[muscle] || ''} 
                                  onChange={(e) => setNewCustomExercise(prev => ({ ...prev, muscles: { ...prev.muscles, [muscle]: parseFloat(e.target.value) || 0 } }))} 
      ...
      {isSaveModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200">
            <h3 className="text-2xl font-black mb-4 text-white">Fechar Semana</h3>
            ...
            <div className="flex gap-4">
              <button onClick={() => setIsSaveModalOpen(false)} className="flex-1 py-4 bg-slate-800 rounded-2xl font-black uppercase text-[10px] tracking-widest text-white hover:bg-slate-700 transition-colors">Cancelar</button>
              <button onClick={handleSaveWeek} className={`flex-1 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest text-white transition-all ${isDeloadActive ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-indigo-600 hover:bg-indigo-500'}`}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
